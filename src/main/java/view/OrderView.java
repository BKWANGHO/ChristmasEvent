    ...
    public static void main(Scanner sc) {
        ...
        while (true) {
            ...
            for (int i = 0; i < controller.orderMenu(order).size(); i++) {
                System.out.println(String.valueOf(controller.orderMenu(order).get(i))
                        + " "
                        + String.valueOf(controller.orderQuantity(order).get(i)) + "개");
            }
            System.out.println("\n<할인 전 총주문 금액>");
            ...
            System.out.println("평일 할인 : -" + decimalFormat.format(controller.weekdayDiscount(date)));
