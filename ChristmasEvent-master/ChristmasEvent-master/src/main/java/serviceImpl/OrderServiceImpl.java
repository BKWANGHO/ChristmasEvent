...
//        total = api +main +dst +bvr;
    ...
    public int weekdayDiscount(OrderModel orderDate) {
        int benefit = 0;
        if (orderModel.getDate() % 7 != 1 || orderModel.getDate() % 7 != 2) {
            for (int i=0; i<orderModel.getMenu().size();i++) {
                if (orderModel.getMenu().get(i).equals("초코케이크")||orderModel.getMenu().get(i).equals("아이스크림")) {
                    benefit += (2023* Integer.parseInt(orderModel.getQuantity().get(i)));
                }
            }
        }
        return benefit;
...
//            메인 개당 -2023원 할인
    ...
    public int specialDiscount(OrderModel orderDate) {
        if (orderDate.getDate() % 7 == 3 || orderDate.getDate() == 25) {
            return 1000;
        }
        return 0;
    }
